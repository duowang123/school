<template>
  <div class="situation">
    ...
    <el-dialog
      ...
      append-to-body
    >
      <div class="course-title" slot="title">
        <span @click="previewVisible = false" class="iconfont icon-arrow-back"></span>课程预览
      </div>
      <Preview
        v-if="previewVisible"
        :course-id="courseId"
...
export default {
  ...
  methods: {
    ...
    updateDistribution(organId) {
      if (this.organId === organId) {
        this.organChange(organId)
      }
    },
    organChange(organId) {
    ...
    handleEdit({ row }) {
      ...
      this.isAdd = false
      const data = this.$_cloneDeep(row)
      this.$nextTick(() => {
        this.$refs.addCourseInfo.initCourseData(data)
      })
    },
    // 新增
    handleAdd() {
