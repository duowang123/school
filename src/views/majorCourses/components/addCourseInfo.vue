<template>
  <div class="addCourseInfo">
    ...
    <!--    </el-scrollbar>-->
    <el-select :disabled="!isAdd" class="organ-select" v-model="selOrganId" @change="updateSpecialty" placeholder="请选择">
      <el-option
        :key="item.id"
        :label="item.name"
        :value="item.id"
    ...
    <el-tabs class="courseInfo" v-model="courseInfoName">
      ...
      <el-tab-pane label="课程详情" name="second">
        ...
        >
          <el-tab-pane
            v-for="(label, name) of tabsCourseData"
            :label="label"
            :name="name"
            :key="name"
          >
    ...
    >
      <Preview
        v-if="renderData.courseId"
        :course-id="courseData.id"
        :tabsCourseData="tabsCourseData"
...
export default {
  ...
  methods: {
    init() {
      ...
      this.selOrganId = this.organId || this.selectableOrganList[0].value
      this.initOptions()
    },
    ...
    initOptions() {
      ...
      api.listByCode({ code: '0016' }).then((res) => {
        this.courseTypeOptions = res.data.map((e) => ({
          label: e.dictName,
          value: e.dictValue
        }))
      })
      api.listByCode({ code: '0018' }).then((res) => {
        this.examModeOptions = res.data.map((e) => ({
          label: e.dictName,
          value: e.dictValue
        }))
      })
      api.listByCode({ code: '0019' }).then((res) => {
        this.examTypeOptions = res.data.map((e) => ({
          label: e.dictName,
          value: e.dictValue
        }))
      })
      api.listByCode({ code: '0017' }).then((res) => {
        this.scoringModelOptions = res.data.map((e) => ({
          label: e.dictName,
          value: e.dictValue
        }))
      })
      api.listByCode({ code: '0020' }).then((res) => {
        this.teachingMethodOptions = res.data.map((e) => ({
          label: e.dictName,
          value: e.dictValue
        }))
      })
    ...
    updateSpecialty() {
      api.getSpecialtyByOrganId({ organId: this.selOrganId }).then(res => {
...
.addCourseInfo {
  ...
  .courseDetails {
    /deep/ > .el-tabs__header > .el-tabs__new-tab {
      height: 28px;
      width: 55px;
      line-height: 28px;
      border: none;
