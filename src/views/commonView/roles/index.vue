  ...
  <div class="role-component">
    <div class="section-left">
      <div class="organ-select" v-if="organType !== 1">
        <el-select style="width: 100%" v-model="organId" @change="organChange" placeholder="请选择">
          <el-option
      ...
      <p class="title"><span class="text">角色列表</span>
        <img @click="addRoleHandler" class="opear" src="@/assets/images/icon_add.png"/>
      </p>
      <div >
        <ul class="role-list">
          <template v-for="role in roles">
            <div :key="role.id" @click="getRoleMenuHandler(role)" class="role-item" :class="{'active': selRoleId===role.id}">
              <li class="role-name" >{{role.roleName}}</li>
              <div class="opear" >
                <img width="16" height="16" @click="updateRole(role)" src="@/assets/images/icon-edit.png" alt="">
                <img width="16" height="16" @click="deleteRole(role)" src="@/assets/images/icon-delete.png" alt="">
              </div>
            </div>
          </template>
        </ul>
    ...
    <div class="section-content">
      <div class="title">
        <div><span class="text">功能权限</span> <span>(包含总院功能)</span></div>
  ...
  export default {
    ...
    methods: {
      ...
      getCheckedId(menu) {
        ...
        console.log(flArr);
        const selId = flArr.filter(e => e.select).map(e => e.id)
