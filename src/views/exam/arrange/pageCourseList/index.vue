      ...
      <div class="main-content-container">
        ...
        >
          <div slot-scope="{ scope }" style="width:200px">
            <span class="opr" @click="handleAttr(scope)">人脸识别记录</span>
          </div>
        </courses-table>
        <pagination
...
export default {
  ...
  methods: {
    ...
    handleAttr({ row }) {
      try {
        this.loading.show()
        api.faceInfoByStudentId(row.studentId).then((res) => {
          this.title = '人脸识别记录'
          ...
          const list = (res.data || []).map((t) => {
            return {
              cardTime: t.cardTime,
              compareResultLabel:
                parseInt(t.compareResult) === 1 ? '成功' : '失败'
            }
          })
          this.componentData = {
            ...
            keys: [
              ...
              { label: '姓名', key: 'realName' },
              {
                ...
                isTable: true,
                tableConfig: [
                  { label: '识别时间', prop: 'cardTime' },
                  { label: '状态', prop: 'compareResultLabel' }
                ]
              }
            ]
          }
    ...
    async getTableData(query) {
      ...
      const res = await api.examPageCourseList(params)
