<template>
  ...
  >
    <el-form-item label="所属考点" prop="testCerterId">
      <el-select v-model="ruleForm.testCerterId" placeholder="请选择">
        <el-option
          v-for="item in openCenterOptions"
          :key="item.value"
          :label="item.label"
          :value="item.value"
        ></el-option>
    ...
    created() {
      if (!this.data.isAdd) {
        ...
        this.ruleForm.testCerterId =
          (this.openCenterOptions.filter(s => this.data.testCerterId === s.value) || [])[0]
            .value || ''
    ...
    data() {
      return {
        ...
        rules: {
          ...
          testCerterId: [{ required: true, message: '请选择', trigger: 'change' }],
