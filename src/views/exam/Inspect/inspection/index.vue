  ...
  <div class="student-plan-right pad20">
    <div>
      <el-form class="user-form" :inline="true">
        ...
        <el-form-item>
          <div class="organ-box">
            <el-select class="organ-select" filterable v-model="params.organId" placeholder="请选择学校">
              <el-option
...
export default {
      ...
      tableConfig: {
        ...
        columnConfig: [
          ...
          },
          {
            ...
            type: 'enums',
            enums: (value) => {
              const arr = vm.inspectionTypeOptions.filter(
  ...
  methods: {
    ...
    initSelectOptions(isRefresh = false) {
      if (!isRefresh) {
        api.listByCode({ code: '0034' }).then((res) => {
          this.inspectionTypeOptions = res.data.map((e) => {
            return {
              id: e.id,
              label: e.dictName,
              value: e.dictValue
    ...
    handlerAdd() {
      ...
      this.componentData = {
        ...
        inspectionTypeOptions: this.inspectionTypeOptions,
      }
    },
    // 编辑
    handleEdit({ row }) {
    ...
    async getTableData(query) {
      ...
      const res = await api.inspectionPageList(params)
    ...
    handleDelete({ row }) {
      this.$confirm('是否继续删除?', '提示', {
        ...
        api.deleteExamInspection({ id: row.id }).then((res) => {
          if (res.code === 200) {
            this.$message({
              type: 'success',
              message: '删除成功!'
