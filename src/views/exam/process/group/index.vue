...
export default {
      ...
      tableConfig: {
        ...
        columnConfig: [
          ...
          },
          {
            ...
            type: 'enums',
            enums: (value) => {
              const arr = vm.examPlanInfoList.filter(
          ...
          },
          {
            ...
            type: 'enums',
            enums: (value) => {
              const arr = vm.stationOptions.filter(
                (item) => item.value === value
              )
              return arr.length > 0 ? arr[0].label : '--'
            }
  ...
  methods: {
    ...
    initSelectOptions(isRefresh = false) {
      if (!isRefresh) {
        api.listByCode({ code: '0035' }).then((res) => {
          this.stationOptions = res.data.map((e) => {
            return {
              id: e.id,
              label: e.dictName,
              value: e.dictValue
    ...
    async getTableData(query) {
      ...
      const res = await api.examGroupPageList(params)
    ...
    handleDelete({ row }) {
      this.$confirm('是否继续删除?', '提示', {
        ...
        api.deleteExamGroup({ id: row.id }).then((res) => {
          if (res.code === 200) {
            this.$message({
              type: 'success',
              message: '删除成功!'
