  ...
  export default {
    ...
    methods: {
      ...
      initSelectOptions(isRefresh = false) {
        !isRefresh && api.listByCode({ code: '0024' }).then(res => {
        ...
        !isRefresh && api.listByCode({ code: '0019' }).then(res => {
          ...
          console.log(this.testTypeOptions);
        })
        ...
        api.coachListByOrganId({ organId: this.params.organId, pageCurrent: 0, pageSize: 100 }).then(res => {
      ...
      async getTableData(query) {
        ...
        const res = await api.invigilatorPageList(params)
      ...
      handleDelete({ row }) {
        this.$confirm('是否继续删除?', '提示', {
          ...
          api.deleteExamInvigilator({ id: row.id }).then((res) => {
            if (res.code === 200) {
              this.$message({
                type: 'success',
                message: '删除成功!'
