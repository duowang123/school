<template>
  <div class="application attrStyle">
    <el-form :rules="rules" :model="ruleForm" ref="addForm" label-width="0">
      <el-form-item label="政策标题" prop="title" class="full-width">
        <el-input v-model="ruleForm.title" placeholder="请输入"></el-input>
      </el-form-item>
      <el-form-item label="政策正文" prop="content" class="full-width">
      ...
      this.$refs.addForm.validate(async (valid) => {
        if (valid) {
          if (!Object.keys(this.data).length) {
            await api.policySave(this.ruleForm).then((res) => {
              if (res.code === 200) {
            ...
            await api.policyUpdate(this.ruleForm).then((res) => {
              if (res.code === 200) {
