<template>
  <div>
    ...
    <el-dialog title="其他费用" :close-on-click-modal="false" append-to-body :visible.sync="dialogAddVisible" width="400px">
      <el-form
        :model="ruleAddForm"
        :rules="rulesAdd"
        ref="ruleAddForm"
...
<script>
  import { addMajorStandard, updateMajorStandard, listByOrganIdAndLevel, viewMajorStandard } from '../api'
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        rules: {
          chargeMoney: [{ required: true, message: '请输入学费标准', trigger: 'blur' }, {
            pattern: '^\\d+$',
            message: '只能是数字',
            trigger: 'blur'
          }],
        ...
        rulesAdd: {
          chargeMoney: [{ required: true, message: '请输入收费标准', trigger: 'blur' }, {
            pattern: '^\\d+$',
            message: '只能是数字',
            trigger: 'blur'
          }],
